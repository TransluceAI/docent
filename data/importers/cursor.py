...
class CursorAgentRunMetadata(BaseAgentRunMetadata):
    general_explanation: str = Field(description="An explanation of the overall rollout structure")
    task_id: str | None = Field(description="Prompt/input into the model", default=None)
    gold_diff: str = Field(description="The reference diff that the model is being judged against")
    judge_prompt: str = Field(description="The prompt that was used to judge the model")
    rollout_diff: str = Field(description="The diff generated by the model in the rollout")
    llm_judge_data: str = Field(description="The response from the judge model")
    rank: int = Field(description="The data-parallel rank of the model")
    step: int = Field(description="The training step number")
    rollout: int = Field(description="The rollout id of the training step")
...
def load_agent_run_from_content(
    ...
    metadata = CursorAgentRunMetadata.model_validate(metadata_dict)

    return AgentRun(transcripts={"default": Transcript(messages=messages)}, metadata=metadata)
...
async def process_cursor_file(file_path: Path) -> Tuple[List[AgentRun], dict[str, Any]]:
    ...
    # Handle directory
        ...
        try:
            ...
            rollout_files = get_all_rollout_files(str(file_path))
            ...
            for rank, step, rollout in rollout_files:
                rollout_path = (
