...
class Datapoint(BaseModel):
    ...
    @classmethod
    def from_transcript(cls, transcript: Transcript) -> Datapoint:
        return cls(
            obj=transcript,
            id=f"{transcript.metadata.task_id}_{transcript.metadata.sample_id}_{transcript.metadata.experiment_id}_{transcript.metadata.epoch_id}".replace(
                "/", "_"
            ),
        )
...
class Judgment(BaseModel):
    ...
    sub_judgments: list[Judgment] | None = None
