...
async def counterfactual_experiment_job(ctx: WorkspaceContext, job: SQLAJob):
    ...
    2. Creates a CounterfactualExperiment instance
    ...
    logger.info(f"Starting counterfactual experiment job for config {experiment_config_id}")

    # Load the experiment configuration
    config = await counterfactual_svc.build_experiment_config(experiment_config_id)
    ...
    def _compute_state_signature(
        result: CounterfactualExperimentResult, subscribed_run_ids: Set[str]
    ) -> Tuple[Any, ...]:
        ...
        - idea output length and tail
        - context outputs: per-id length and tail
        - parsed ideas: count and ids
        - agent_run_metadata: per-run (state, grade)
        - collection id
        - subscribed runs lightweight transcript signal: per-run (num transcripts, per transcript message count and tail length)

        TODO(neil): this is somewhat hacky and an indirect way of getting what we want.
        """
        ...
        idea_out = result.counterfactual_idea_output or ""
        idea_sig: Tuple[int, str] = (len(idea_out), idea_out[-16:])

        ctx_out = result.counterfactual_context_output or {}
        # Stable order by key
        ctx_sig: Tuple[Tuple[str, int, str], ...] = tuple(
            (k, len(v or ""), (v or "")[-16:]) for k, v in sorted(ctx_out.items())
        )

        parsed = result.parsed_counterfactual_ideas
        if parsed and parsed.counterfactuals:
            parsed_ids: Tuple[str, ...] = tuple(sorted(parsed.counterfactuals.keys()))
        else:
            parsed_ids = tuple()
    ...
    async def _maybe_publish_state(result: CounterfactualExperimentResult, *, force: bool = False):
        ...
        try:
            ...
            # Convert to CounterfactualExperimentSummary for frontend
    ...
    # Run the experiment
    experiment = CounterfactualExperiment(config)
    final_result = None
    ...
    try:
        ...
        async for result in experiment.run():
            # Stream each update to Redis, in case anything has changed
        ...
        if final_result:
            try:
                ...
                result_id = await counterfactual_svc.store_experiment_result(
        ...
        if final_result and final_result.experiment_status.status == "error":
            ...
            try:
                ...
                if raw:
                    summary = CounterfactualExperimentSummary.model_validate_json(raw)
