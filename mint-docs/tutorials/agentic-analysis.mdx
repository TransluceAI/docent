...
Coding agents can flexibly automate multi-step workflows that are time-consuming when manual or beneficial to conduct in parallel. For instance, your agent can support:

- **Comparing between checkpoints**. Prompt the agent to identify what changed between multiple versions of a model. In order to answer questions like "What caused a regression between checkpoint A and checkpoint B?" or "These two checkpoints show a quantitative tradeoff on eval scores. What behaviors might explain that?", the agent can compare a successful and failed run against each other at scale.
- **Triage before deep analysis**: Your agent can triage runs before engaging in analysis at scale, improving cost efficiency. Focus expensive analysis by prompting the agent to:
  ...
  2. Then use a more expensive model with detailed, issue-specific rubrics over only the relevant parts of the collection. Assign a detailed sycophancy judge to run over the transcripts tagged "sycophancy" and a copyright judge to run over the transcripts tagged "copyright."
...
<Tabs>
  <Tab title="Cursor Template">
    ...
    [Click here](/assets/cursor-quickstart.zip) to download our template folder. Open it in a new Cursor workspace.
...
- We're currently adding functionality to analyze judge results. Stay tuned!
