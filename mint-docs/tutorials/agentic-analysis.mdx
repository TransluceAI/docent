...
The Docent Agent can flexibly support complex workflows that are time-consuming to implement manually or beneficial to conduct in parallel. For instance, the agent can support with:

- **Comparing between checkpoints**. The Docent agent can help identify what changed between multiple versions of a model. In order to answer questions like "What caused a regression between checkpoint A and checkpoint B?" or "These two checkpoints show a quantitative tradeoff on eval scores. What behaviors might explain that?", the agent can compare a successful and failed run against each other at scale.
- **Triage before deep analysis**: The Docent agent can triage runs before engaging in analysis at scale, improving cost efficiency. For instance, the Docent agent might do a pre-screen with a cheap model that tags transcripts with issues like "sycophancy," "copyright," or "biorisk." A judge with a more detailed rubric and expensive model can then grade harms on only the most suspicious runs.
- **Managing long context**: In collections with long traces or a large volume of transcripts, the Docent agent supports a range of methods for supporting context management. The agent can recursively summarize extracting key observations from each batch, and then observe patterns in the key observations. Recursively passing the results from previous analyses lets the agent handle collection-level questions over large collections of data.

The Docent Agent writes a script saved to your local directory, which you can modify for fine-grained control of the workflow. You can then rerun the script to repeat the analysis with your modifications.
...
<Tabs>
  <Tab title="Cursor Template">
    ### Step 1: Download our template folder and open it in Cursor.

    Download the [template folder](https://drive.google.com/file/d/1JDUi5jLkwJrHzUzBVl5mwHHDjKwfrByP/view?usp=sharing) and open it in Cursor.

    The `agentic_analysis_quickstart/` directory contains:
    ...
    - **`.cursor/rules/docent.md`** and **`.cursor/mcp.json`**: configuration files for Cursor defining an MCP server and outlining rules for how the agent should use Docent.
    ...
    2. The `DOCENT_COLLECTION_ID` is the UUID of the collection you want to analyze. You can copy it quickly from the collections table in the Docent Dashboard by clicking on the ID next to the collection you want to analyze. If you are within a collection, the ID will be available next to the title.
  ...
  <Tab title="Existing Workspace">
    ...
    This should be in the same directory as your pyproject.toml
    ...
    For Cursor, download the [Docent Instructions](https://drive.google.com/file/d/1ZdlsT4RVsIRIHxOKq8-knYIOfel3tVhL/view?usp=sharing) and add them to `.cursor/rules`.
...
- We're developing a more intentional workflow for analyzing judge results.
