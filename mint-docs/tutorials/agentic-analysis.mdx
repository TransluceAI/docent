...
- **Comparing between checkpoints**. Suppose you want to answer questions like "What caused a regression between checkpoint A and checkpoint B?" or "These two checkpoints show a quantitative tradeoff on eval scores. What behaviors might explain that?" You can now prompt the agent to compare pairs of successful and failed runs in order to identify changes between multiple versions of a model.
- **Triaging before deep analysis**: Improve cost-efficiency by focusing your expensive classifier runs. You can prompt your agent to:
