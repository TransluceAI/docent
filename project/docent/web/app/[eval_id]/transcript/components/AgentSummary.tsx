...
const AgentSummary: React.FC<AgentSummaryProps> = ({
  ...
  useEffect(() => {
    ...
    if (datapoint?.obj.metadata.is_loading_messages) {
      console.log('Clearing action summary because datapoint is still loading');
      clearActionsSummary();
    }
    // If we're loading summaries for a different datapoint, clear the summaries
    if (
      loadingActionsSummaryFor &&
      loadingActionsSummaryFor !== datapoint?.id
    ) {
      clearActionsSummary();
    }
    // Otherwise, just request the summaries for the current datapoint
    if (datapoint?.id) {
      requestActionsSummary(datapoint.id);
    }
