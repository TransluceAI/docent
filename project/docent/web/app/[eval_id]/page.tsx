...
function DocentDashboardContent() {
  ...
  const { curEvalId } = useFrameGrid();

  const handleShowDatapoint = (datapointId: string, blockIdx?: number) => {
    if (blockIdx !== undefined) {
      router.push(
        `${BASE_DOCENT_PATH}/${curEvalId}/transcript/${datapointId}?block_id=${blockIdx}`
      );
    } else {
      router.push(`${BASE_DOCENT_PATH}/${curEvalId}/transcript/${datapointId}`);
    }
  };
