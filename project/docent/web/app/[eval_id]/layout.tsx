...
import { ApiKeyModal } from '../components/ApiKeyManager';
...
function DocentLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const { isConnected, startNewEval } = fg;
  const params = useParams();
  const evalId = params.eval_id as string;

  // Fetch state from the server
  const fetchRef = React.useRef(false); // Dedup
  useEffect(() => {
    if (!isConnected || !evalId || fetchRef.current) {
      return;
    }
