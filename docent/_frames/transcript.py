...
METADATA_FIELDS_TO_DUMP = ["scores", "additional_metadata", "scoring_metadata"]
METADATA_FIELDS_TO_INDEX = ["task_id", "sample_id", "epoch_id", "experiment_id", "model", "scores"]
...
class Transcript(BaseModel):
    ...
    def validate_metadata_fields(cls, metadata: TranscriptMetadata):
        ...
        if invalid_fields:
            raise ValueError(
                f"The following metadata fields to dump do not exist on TranscriptMetadata: {invalid_fields}"
            )
    ...
    def to_str(
        self,
        metadata_fields: list[str] | None = METADATA_FIELDS_TO_DUMP,
